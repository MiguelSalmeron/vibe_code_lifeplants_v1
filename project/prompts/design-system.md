# Design System
- Global theme lives in `app/styles/theme.css`.
- Prefer existing tokens from `app/styles/tokens/`.
- Add new semantic variables only when the current set cannot express the design cleanly.
## Visual direction
- Keep the system expressive, organic, and readable.
- Use typography, spacing, and color to signal hierarchy clearly.
- Avoid generic or flat layouts when a more intentional composition is possible.
## Usage notes
- Use tokens before hard-coded values.
- Keep component-level styling consistent with the theme.
- Document any new visual rule that affects multiple surfaces.
# Design System
